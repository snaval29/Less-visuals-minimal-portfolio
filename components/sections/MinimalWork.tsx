  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
                ...
                >
                    <a href={project.link} target="_blank" rel="noopener noreferrer" className="block transform-gpu">
                        <div className="flex flex-col lg:grid lg:grid-cols-12 gap-8 lg:gap-20 items-center">
                            ...
                            {/* Image Container */}
                            <div className="w-full lg:col-span-7 relative aspect-[4/3] md:aspect-[16/10] bg-neutral-50 overflow-hidden rounded-[1.5rem] md:rounded-[3rem] border border-neutral-100 will-change-transform">
                                <Image 
                                    src={project.image} 
                                    alt={project.name}
                                    fill 
                            ...
                            <div className="w-full lg:col-span-5 flex flex-col">
                                ...
                                <p className="text-neutral-600 text-base md:text-xl leading-relaxed mb-6 md:mb-8 font-medium tracking-tight">
                                    {project.description}
                                </p>
                                
                                <div className="flex items-center gap-3 text-black font-bold text-[10px] md:text-xs uppercase tracking-widest active:translate-x-1 transition-transform">
        ...
        {/* --- ENGINEERING SECTION --- */}
        <div className="border-t border-neutral-100 pt-16 md:pt-32">
            ...
            {codeProjects.map((project, index) => (
                <div key={index} className="bg-neutral-50/50 border border-neutral-100 p-6 md:p-14 rounded-[1.5rem] md:rounded-[3rem] hover:bg-white hover:shadow-xl hover:shadow-neutral-200/50 transition-all duration-500 transform-gpu">
                    <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8 md:gap-10">
                        <div className="max-w-2xl">
                            <div className="flex items-center gap-4 mb-3 md:mb-4">
                                <h3 className="text-2xl md:text-4xl font-bold text-black tracking-tighter">{project.name}</h3>
                                <span className="bg-black text-white text-[8px] md:text-[9px] font-bold px-2 md:px-3 py-1 rounded-full uppercase tracking-widest">Plugin</span>
                            </div>
                            <p className="text-neutral-500 text-base md:text-lg font-medium leading-relaxed tracking-tight">
                                {project.description}
                            </p>
                        </div>
                        <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
                            <a href={project.figmaLink} target="_blank" className="flex items-center justify-center gap-2 px-6 py-3 md:px-8 md:py-4 bg-black text-white font-bold text-[10px] md:text-xs rounded-xl md:rounded-2xl hover:bg-neutral-800 active:scale-95 transition-all tracking-widest uppercase">
                                <Figma size={16} /> Try Plugin
                            </a>
                            <a href={project.githubLink} target="_blank" className="flex items-center justify-center gap-2 px-6 py-3 md:px-8 md:py-4 bg-white border border-neutral-200 text-black font-bold text-[10px] md:text-xs rounded-xl md:rounded-2xl hover:border-black active:scale-95 transition-all tracking-widest uppercase">
